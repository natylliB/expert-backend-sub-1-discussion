...
describe('DomainErrorTranslator', () => {
  it('should translate error correctly', () => {
    ...
    expect(DomainErrorTranslator.translate(new Error('USER_CREDENTIAL.NOT_CONTAIN_REQUIRED_PROPERTY')))
      .toStrictEqual(new InvariantError('tidak dapat login karena properti yang dibutuhkan tidak ada'));
    expect(DomainErrorTranslator.translate(new Error('USER_CREDENTIAL.PROPERTY_NOT_MET_DATA_TYPE_SPECIFICATION')))
      .toStrictEqual(new InvariantError('tidak dapat login karena tipe data tidak sesuai'));
    expect(DomainErrorTranslator.translate(new Error('USER_CREDENTIAL.USERNAME_LIMIT_CHAR_EXCEEDED')))
      .toStrictEqual(new InvariantError('tidak dapat login karena karakter username melebihi batas limit'));
    expect(DomainErrorTranslator.translate(new Error('USER_CREDENTIAL.USERNAME_CONTAIN_RESTRICTED_CHARACTER')))
